...
SEC("cgroup/bind4")
int filter_bind(struct bpf_sock_addr *sk_addr) {
#define BPF_LOG_TOPIC "bind4"
    u64 cg = bpf_get_current_cgroup_id();
    u32 ip4 = sk_addr->user_ip4;
    bpf_log_debug("bind4 %pI4:%d cg:%d", &ip4, bpf_ntohs(sk_addr->user_port), cg);
    if (!sk_addr->user_port) {
        sk_addr->user_port = bpf_htons(10000);
    }

    return (BPF_RET_BIND_NO_CAP_NET_BIND_SERVICE << 1) | 1;
#undef BPF_LOG_TOPIC
}

SEC("cgroup/post_bind4")
int filter_post_bind(struct bpf_sock *sk) {
#define BPF_LOG_TOPIC "post_bind4"
    u64 cg = bpf_get_current_cgroup_id();
    bpf_log_debug("bind4 %pI4:%d cg:%d", sk->src_ip4, sk->src_port, cg);
#undef BPF_LOG_TOPIC
    return 1;
}
